...
import edu.
colorado.phet.bendinglight.BendingLightSimSharing.UserComponents;
import edu.
colorado.phet.common.phetcommon.model.property.Property;
import edu.
colorado.phet.common.phetcommon.util.SimpleObserver;
import edu.
colorado.phet.common.piccolophet.nodes.WavelengthControl;
import edu.
...
import static edu.
colorado.phet.bendinglight.view.LaserColor.WHITE_LIGHT;
import static edu.
...
{
  public
  BendingLightWavelengthControl( final
  Property < Double > wavelengthProperty, final
  Property < LaserColor > laserColor
)
  {
    final
    WavelengthControl
    wavelengthControl = new WavelengthControl( UserComponents.wavelengthControl, false, 150, 27, MIN_WAVELENGTH, 700 )
    {
      {//only go to 700nm because after that the reds are too black
        final
        PNode
        wc = this;//to access from within closure below
        ...
        laserColor.addObserver( new SimpleObserver()
        {
          ...
          void update()
          {
            final
            boolean
            disabled = laserColor.get() == WHITE_LIGHT;
            wc.setTransparency( disabled ? 0.3f
          :
            1
            f
          )
    ...
    ;
    PBounds
    bounds = wavelengthControl.getFullBounds();
