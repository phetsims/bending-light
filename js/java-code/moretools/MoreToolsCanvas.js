...
import edu.
colorado.phet.bendinglight.BendingLightStrings;
import edu.
colorado.phet.bendinglight.model.ProtractorModel;
import edu.
colorado.phet.bendinglight.modules.intro.IntroCanvas;
import edu.
colorado.phet.bendinglight.modules.intro.IntroModel;
import edu.
colorado.phet.bendinglight.view.BendingLightCanvas;
import edu.
colorado.phet.bendinglight.view.BendingLightWavelengthControl;
import edu.
colorado.phet.bendinglight.view.LaserView;
import edu.
colorado.phet.bendinglight.view.ProtractorNode;
import edu.
colorado.phet.common.phetcommon.math.vector.Vector2D;
import edu.
colorado.phet.common.phetcommon.model.ResetModel;
import edu.
colorado.phet.common.phetcommon.model.Resettable;
import edu.
...
import edu.
colorado.phet.common.phetcommon.model.property.Or;
import edu.
colorado.phet.common.phetcommon.model.property.Property;
import edu.
colorado.phet.common.phetcommon.model.property.ValueEquals;
import edu.
colorado.phet.common.phetcommon.util.Option;
import edu.
colorado.phet.common.phetcommon.util.SimpleObserver;
import edu.
colorado.phet.common.phetcommon.util.function.Function0;
import edu.
colorado.phet.common.phetcommon.util.function.Function1;
import edu.
colorado.phet.common.phetcommon.util.function.Function3;
import edu.
colorado.phet.common.phetcommon.util.function.VoidFunction0;
import edu.
colorado.phet.common.phetcommon.util.function.VoidFunction1;
import edu.
colorado.phet.common.phetcommon.view.graphics.transforms.ModelViewTransform;
import edu.
colorado.phet.common.piccolophet.nodes.VelocitySensor;
import edu.
colorado.phet.common.piccolophet.nodes.VelocitySensorNode;
import edu.
colorado.phet.common.piccolophet.nodes.toolbox.NodeFactory;
import edu.
colorado.phet.common.piccolophet.nodes.toolbox.ToolIconNode;
import edu.
umd.cs.piccolo.PNode;

import static edu.
colorado.phet.bendinglight.view.ToolboxNode.ICON_WIDTH;
import static edu.
...
public
MoreToolsCanvas( MoreToolsModel
model, BooleanProperty
moduleActive, Resettable
resetAll
)
{
...
protected
PNode[ ]
getMoreTools( ResetModel
resetModel
)
{
  //Create the Velocity Sensor tool and wave sensor tool to add to the toolbox
  return new PNode[ ]
  { createVelocitySensorTool( resetModel ), createWaveSensorTool( resetModel ) }
  ;
}

private
ToolIconNode < BendingLightCanvas < MoreToolsModel >> createWaveSensorTool( ResetModel
resetModel
)
{
  //Create a model for depicting with the WaveSensorNode
  final
  Function1.Constant < Vector2D, Option < Double >> value = new Function1.Constant < Vector2D, Option < Double >> ( new Option.None < Double > () );//Dummy function that always returns None
  final
  WaveSensor
  waveSensor = new WaveSensor( new ConstantDtClock(), value, value );

  //Create the WaveSensorNode
  final
  WaveSensorNode
  waveSensorNode = new WaveSensorNode( transform, waveSensor );
  ...
  final
  int
  waveToolHeight = (int)( waveSensorNode.getFullBounds().getHeight() / waveSensorNode.getFullBounds().getWidth() * ICON_WIDTH );

  //Provide a way of generating the WaveSensorNode when dragged out of the toolbox
  final
  NodeFactory
  waveNodeFactory = new NodeFactory()
  {
    public
    WaveSensorNode
    createNode( ModelViewTransform
    transform, final
    Property < Boolean > showTool, final
    Point2D
    modelPt
  )
    {
...
private
ToolIconNode
createVelocitySensorTool( ResetModel
resetModel
)
{
  //Create the VelocitySensorNode to depict in the toolbox
  final
  Function1 < Vector2D, String > formatter = new Function1 < Vector2D, String > ()
  {
    public
    String
    apply( Vector2D
  ...
  )
    {
      final
      String
      value = new DecimalFormat( "0.00" ).format( velocity.magnitude() / 2.99792458E8 );
  ...
  final
  NodeFactory
  velocityNodeFactory = new NodeFactory()
  {
    public
    VelocitySensorNode
    createNode( final
    ...
    {
      model.velocitySensor.position.set( new Vector2D( modelPt ) );
      return new VelocitySensorNode( transform, model.velocitySensor, arrowScale, new Property < Function1 < Vector2D, String >> ( formatter ), getBoundedConstraint(), PICCOLO_PHET_VELOCITY_SENSOR_NODE_UNKNOWN, new Vector2D( 1.00, 0 ) )
      {
        {
  ...
  final
  VelocitySensorNode
  thumbnailSensorNode = new VelocitySensorNode( transform, new VelocitySensor(), arrowScale, new Property < Function1 < Vector2D, String >> ( formatter ), new Vector2D( 1.00, 0 ) );
  final
  int
  velocityToolHeight = (int)( thumbnailSensorNode.getFullBounds().getHeight() / thumbnailSensorNode.getFullBounds().getWidth() * ICON_WIDTH );
...
private
Function0 < Rectangle2D > getToolboxBounds()
{
...
protected
ProtractorNode
newProtractorNode( ModelViewTransform
transform, Property < Boolean > showTool, Point2D
model
)
{
