...
import edu.
colorado.phet.common.phetcommon.model.clock.Clock;
import edu.
...
WaveSensor
{
  ...
  final
  double
  DELTA = 1;//offset the probe so it isn't taking data by default
  ...
  final
  Probe
  probe1 = new Probe( -4.173076923076922E-7 - DELTA, 9.180769230769231E-7 - DELTA );
  public
  final
  Probe
  probe2 = new Probe( -1.5440384615384618E-6 - DELTA, -1.2936538461538458E-6 - DELTA );
  public
  final
  Property < Vector2D > bodyPosition = new Property < Vector2D > ( new Vector2D( 4.882500000000015E-6 - DELTA, -3.1298076923077013E-6 - DELTA ) );

  public
  final
  Clock
  clock;//Clock to observe the passage of time
  public
  final
  BooleanProperty
  visible = new BooleanProperty( false );//in the play area
  ...
  clock,
    final
  Function1 < Vector2D, Option < Double >> probe1Value,//Function for getting data from a probe at the specified point
    final
  Function1 < Vector2D, Option < Double >> probe2Value
)
  {
    this.clock = clock;
    ...
    {
      public
      void simulationTimeChanged( ClockEvent
      clockEvent
    )
      {
  ...
  //Read samples from the probes when the simulation time changes
  private
  void updateProbeSample( Probe
  probe, Function1 < Vector2D, Option < Double >> probeValue, Clock
  clock
)
  {
    //Read the value from the probe function.  May be None if not intersecting a light ray
    final
    Option < Double > value = probeValue.apply( probe.position.get() );
    if ( value.isSome() ) {
  ...
  public
  void translateBody( Dimension2D
  delta
)
  {
  ...
  public
  void translateToHotSpot( Point2D
  position
)
  {
...
class
  Probe
  {
    ...
    public
    final
    Property < ArrayList < Option < DataPoint >>> series = new Property < ArrayList < Option < DataPoint >>> ( new ArrayList < Option < DataPoint >> () );
    ...
    public
    void addSample( final
    Option < DataPoint > sample
  )
    {
